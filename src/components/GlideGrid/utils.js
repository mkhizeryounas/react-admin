...
export function useMockDataGenerator(numCols, readonly = true, group = false) {
  ...
  const setCellValueRaw = React.useCallback(([col, row], val) => {
    cache.current.set(col, row, val);
  }, []);
    ...
    ([col, row], val) => {
      ...
      if (isEditableGridCell(val) && isEditableGridCell(current)) {
        ...
        cache.current.set(col, row, {
          ...copied,
          displayData:
            typeof copied.data === 'string' ? copied.data : copied.displayData,
    ...
    },
    {
      ...
      getContent: () => {
        return {
          kind: GridCellKind.Image,
          data: [
            `${faker.image.animals(40, 40)}?random=${faker.datatype.number(
              100_000
            )}`,
          ],
          allowOverlay: true,
          ...
          data: [
            {
              ...
              img: `${faker.image.nature(
                40,
                40
              )}?random=${faker.datatype.number(100_000)}`,
            },
            {
              text: faker.address.cityName(),
              img: `${faker.image.nature(
...
export function useAllMockedKinds() {
  ...
  const setCellValue = React.useCallback(
    ([col, row], val, noDisplay, forceUpdate) => {
      let current = cache.current.get(col, row);
      if (current === undefined) {
        current = colsMap[col].getContent();
      }
      if (isEditableGridCell(val) && isEditableGridCell(current)) {
        ...
        cache.current.set(col, row, {
          ...copied,
          displayData:
            noDisplay === true ? undefined : copied.data?.toString() ?? '',
        });
